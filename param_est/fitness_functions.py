...
def auxin_peak_at_root_tip(sim: GrowingSim, chromosome: dict) -> float:
    ...
    return avg_root_tip_auxins/avg_non_root_tip_auxins

def auxin_greater_in_larger_cells(sim: GrowingSim, chromosome: dict) -> float:
    """
    Returns the correlation coefficient between cell size and auxin concentration in meristematic and transition XPP cells.
